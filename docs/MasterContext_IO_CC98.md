# Refueler Master Context — IO CC-98
*Updated: 2026-08-18 (TDP-C — Opus uncounted. `update-lightning-address` v1 deployed (S-LN-1 closed). NumoPay fork alignment findings. BRIDGE v4.7. Session closed.)*
*Supersedes: MasterContext_IO_CC97*
...
**Fiat commission:** Stripe off-session PaymentIntent (4–8% of order value). Settles to bank account — Revolut Business recommended.
...
**Sidebar:** removed. 340px returned to queue. Mapbox dependency removal deferred TDP-C.
...
- JWT validated via `getUser()`. Owner PIN verified server-side via bcrypt (never trust the browser gate alone).
...
```
Scoped for Menu Management v1.

---

## NumoPay fork — alignment findings (TDP-C)

**Base:** cashubtc/Numo v1.8. Package `io.refueler.merchant`. Hardening phases 1–3 complete.
**Current state:** Clean v1.8 base, no Supabase integration yet. BRIDGE v4.7 committed to repo (`be2e106` + rebase pending).

**Capabilities confirmed in fork:**
- `OnboardingActivity`, `ModernPOSActivity`, `PaymentRequestActivity`, `PaymentReceivedActivity`
- `PinEntryActivity` / `PinSetupActivity` / `PinResetActivity` — native PIN security
- `ItemListActivity` / `ItemEntryActivity` / basket system — item catalogue built natively
- `WebhookSettingsActivity` — webhook config UI exists
- `InsightsActivity` / `PaymentsHistoryActivity` — history and analytics
- `AutoWithdrawSettingsActivity` — auto-withdrawal threshold

**Stack:** OkHttp3 + Jackson + Gson. `cdk-android:0.17.2-rc.1` (note -rc.1).

**NumoPay-A agenda:**

| Item | Decision needed |
|---|---|
| Auth model | Replace/wrap `OnboardingActivity` — Supabase magic link + staff PIN. Screen-on flag. No mid-shift re-auth. |
| Payment routing | Order entry → `merchant_orders` via Supabase. Not Cashu melt for floor orders. |
| Item catalogue | Supabase `merchant_menu_items`. Study NumoPay's native catalogue UI/UX first. |
| Noun/verb/handle taxonomy | Order code as universal join key. |
| Android theming | Map Carbon tokens to `themes.xml` / `colors.xml` / `dimens.xml`. |
| CDK version | Confirm `cdk-android:0.17.2-rc.1` vs stable before any Cashu work. |
| **Manual attach at open** | `OnboardingActivity.kt`, `ModernPOSActivity.kt`, `WebhookSettingsActivity.kt` |
...
**Permanently closed:** ~~S-LN-1~~ (CC-98) · ~~S-12~~ · ~~S-14~~ · ~~S-18~~ · ~~S-23~~ · ~~S-24~~ · ~~S-25~~ · ~~S-26~~ · ~~S-27~~
...
- Push BRIDGE v4.7 to `numo-fork/` (re-run after rebase), `refueler-share/`, `refueler-legend/`, `refueler-pass/`, `refueler-io/docs/`
...
|---|---|---|---|---|
| 1 | **NumoPay-A** | Fork review, auth model, API contract, noun/verb/handle taxonomy | Opus uncounted | **Next** |
...
| — | **Menu Management v1** | CSV import, time-based menus, menu-item primitive | Sonnet counted | After NumoPay-A |
...
| — | **Staff Management v1** | Per-staff accounts, AM Blink wallet, ops monitoring via blink-balance EF | Sonnet counted | Gap — informed by NumoPay-A |
...
| — | **September User Guide update** | Lightning address change section, anti-phishing panel, AM onboarding checklist | Sonnet counted | September |
