# Refueler Master Context — IO CC-89
*Updated: 2026-08-16 (CC-89 — Sonnet counted. cc89_pin_bcrypt migration applied. bcrypt columns added to merchant_users. Terminal still on SHA-256 path — CC-90 cuts over to verify-pin Edge Function.)*
*Supersedes: MasterContext_IO_CC88*
...
|---|---|---|---|
| S-18 | PIN auth server-side: CC-89 complete (bcrypt columns live). CC-90: safe view, verify-pin Edge Function, JS cutover, full sim | High | CC-90 — next counted session |
...
| 1 | **CC-90 — S-18 Steps 2–6** | Safe view + revoke direct SELECT, verify-pin Edge Function (bcrypt wf12, rate-limit 5/5min), JS cutover, full sim | Sonnet counted | **Next** |
...
## PIN hash status — S-18 (CC-89 / CC-90)

**Migration `cc89_pin_bcrypt` applied 2026-08-16:**
- `merchant_users.staff_pin_bcrypt TEXT` — bcrypt wf12, nullable
- `merchant_users.owner_pin_bcrypt TEXT` — bcrypt wf12, nullable
- Raj's Steakhouse seeded: staff 1234, owner 8888, both `$2b$12$` prefix confirmed
- Old columns `staff_pin_hash` / `owner_pin_hash` retained until Sim-Close

**CC-90 remaining (S-18 Steps 2–6):**
- Migration `cc90_merchant_users_safe_view`: view excluding all hash columns, revoke direct SELECT from `anon` + `authenticated`, grant SELECT on view only
- Edge Function `verify-pin`: bcrypt compare, JWT validation, per-user_id rate-limit 5 attempts/5 min, returns `{ valid: true|false }` only, `verify_jwt: false`
- `merchant-tablet-logic.js`: remove `sha256()`, remove hash state vars, point resolveVenueAndPins at `merchant_users_safe` view, replace verifyStaffPin/verifyOwnerPin with fetch to verify-pin, add 5-attempt client-side lockout (30s)
- Full sim: magic link → staff PIN → owner PIN → wrong PIN ×5 → lockout

**Logged CC-89:**
- PIN reset UI routes through a dedicated Edge Function (bcrypt server-side) — not client-side hash. One-function job when Owner tab PIN reset is scoped.
- Hardening-A (Opus uncounted, after Sim-Close): `anon` has blanket table-level grants on `merchant_users` — first Hardening-A item. Full audit informed by SECURITY-RESEARCH-LOG.md.
