# Refueler Master Context — IO CC-94
*Updated: 2026-08-17 (Hardening-A — Sonnet counted. Supabase-wide RLS and grant audit complete. Six migrations applied. G-4 cleared. Three deprecated telemetry tables dropped. SHA-256 PIN columns dropped. venue_partners anon grant surface eliminated. orders→venue_partners FK added.)*
*Supersedes: MasterContext_IO_CC93*
...
## Pre-merchant gate list (updated CC-94)
...
| G-4 | **Hardening-A** | ✅ **CLEARED CC-94** | Six migrations applied. anon grant surface eliminated. SHA-256 columns dropped. Telemetry tables purged. FK added. |
| G-5 | S-26 (orders→venue_partners FK) | ✅ **CLEARED CC-94** | FK added via `hardening_a_orders_venue_fk`. |
...
## Supabase — schema state (post-CC-94)

**Migrations CC-94 (Hardening-A):**
- `hardening_a_merchant_users_grants` — revoked anon/authenticated write grants on `merchant_users`; dropped duplicate SELECT policy
- `hardening_a_venue_partners_revoke_anon_addresses` — initial column revoke (superseded)
- `hardening_a_venue_partners_grants_clean` — full `REVOKE ALL` on `venue_partners` from anon/authenticated; re-granted `SELECT, UPDATE TO authenticated` only
- `hardening_a_orders_venue_fk` — `orders_venue_id_fkey` FK added
- `hardening_a_remove_sha256_pin_columns` — `staff_pin_hash`, `owner_pin_hash` dropped
- `hardening_a_drop_deprecated_telemetry` — `log_entries`, `live_transactions`, `sessions` dropped

**Confirmed post-migration state:**
- `merchant_users`: anon holds zero grants. authenticated holds column-level SELECT on 6 safe columns only (no PIN columns). bcrypt columns (`staff_pin_bcrypt`, `owner_pin_bcrypt`) are service_role-only.
- `venue_partners`: anon holds zero grants (table + column). authenticated holds SELECT + UPDATE only. Wallet columns (`lightning_address`, `onchain_address`, `silent_payment_address`) unreachable by anon.
- `orders_venue_id_fkey` FK present. 21 orders, 0 orphans at time of migration.
- `merchant_users_safe` view: `security_invoker=true` confirmed. anon SELECT on view returns zero rows (no row policy).
- Deprecated tables (`log_entries`, `live_transactions`, `sessions`) purged. `orders.session_id` column retained; FK constraint dropped by cascade.
...
- Raj's Steakhouse: staff PIN 1234, owner PIN 8888, coords live, active true, `lightning_address = 'trickdraw318@walletofsatoshi.com'`
...
Pre-pivot telemetry tables (`log_entries`, `live_transactions`, `sessions`) dropped CC-94. Dev console data sources are now null. Re-scope around real operational metrics: orders placed/confirmed/settled (`orders` + `merchant_orders`), Blink wallet balance (`blink-balance` Edge Function), active venue count (`venue_partners.active`), Edge Function invocation counts, and future product telemetry (sats rewarded, stamp issuances, Legend queries). Scope within TDP-A or as a lightweight standalone session after TDP-A.
...
**Permanently closed:**
- ~~S-12~~ `car_park_occupancy` strip · ~~S-14~~ Costa label · ~~S-18~~ PIN auth · ~~S-23~~ Queue sign-out · ~~S-24~~ favicon/PWA metas · ~~S-26~~ orders→venue_partners FK

---

## Queued sessions — forward plan (post-Hardening-A)

| # | Session | Scope | Type | Status |
|---|---|---|---|---|
| 1 | **TDP-A** | Terminal Design Philosophy — audit, comparators, primitives. Dev console re-scope to be incorporated. | Opus uncounted | **Next** |
...
- `venue_partners.active` toggle — add to Command Centre or Owner tab (TDP-B)
