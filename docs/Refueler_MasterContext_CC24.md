...
**Latest commits:** `d975093` (CC-23 — Command Centre files moved to repo root) + anon key fix commit (CC-23 close) + CC-24 BTC tile + Darwin feed commit + `CC-24b` sat/vB removal
...
| `dev-console.html` | **repo root** ✅ | CC-24 — Blink wallet tile, BTC Clark Moody format. Auth gate buttons orange ⚠️ — fix queued. |
| `franchise-dashboard.html` | **repo root** ✅ | Auth gate buttons orange ⚠️ — fix queued |
| `merchant-tablet.html` | **repo root** ✅ | CC-24 — Darwin live feed via huxley2.unop.uk/FST, 30s polling |

---

## Dev console — telemetry tile state (CC-24)

Current 2×3 grid:

| Tile | Status | Notes |
|---|---|---|
| BLINK WALLET | 🟡 PENDING | Renamed from ZBD. Fetches via `blink-balance` edge function (not yet deployed). Shows PENDING badge until deployed. CC-25 scope. |
| BTC / GBP | ✅ LIVE | `£xx,xxx` primary. `£1 = x,xxx sats` secondary (Clark Moody style). `$` value removed. `sat/vB` removed (CC-24b). CoinGecko API. |
...
- Displays: `x,xxx sats` primary, `≈ £x.xx` meta
...
| CC-24 | BTC tile: `$` removed, `£1 = x,xxx sats` format, `sat/vB` added then removed (CC-24b — too much info). ZBD tile renamed to Blink Wallet; `fetchBlinkBalance()` via `blink-balance` edge function (PENDING deploy). Darwin live feed integrated: `huxley2.unop.uk/departures/FST/3`, 30s polling, populates Horizon Band + sidebar rows + `qs-next-train`. |

---

## Standing items — carry to CC-25

| Item | Priority | Notes |
|---|---|---|
| **Deploy `blink-balance` edge function** | 🔴 CC-25 | Calls `api.blink.sv/graphql` server-side with `BLINK_API_KEY`. Returns `{ balanceSats, balanceGbp }`. Auth-gate with Bearer session token. Follow `bolt11-create-invoice` pattern. |
| **`merchant_orders` schema drift** | 🔴 CC-25 | `payment_status` + `amount_gbp` missing from live table — reconcile before live payments |
...
| **`dev-console.html` / `franchise-dashboard.html` auth buttons** | 🟡 | CSS in dev-console.html already appears correct (outlined gold) — confirm visually in CC-25 |
