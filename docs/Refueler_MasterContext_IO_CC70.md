# Refueler Master Context — IO CC-70
*Updated: 2026-07-25 (CC-70 planning)*
*Supersedes: Refueler_MasterContext_IO_CC69.md*
*Sync log: MasterContext_IO_CC70 — Planning session. Fiat→sats rewards added as Block 8 (primary traction lever). LNBits added as Block 9 (deferred post merchant onboarding — not economically viable until 1–2 merchants live). LNBits prep work approved (document webhook swap points only, no VPS yet). Hetzner CX22 €4.50/month costed for future reference. Ticketing proto-concept confirmed (8-char reference → one-time scannable token via BoltCard/LNURL-withdraw). Ben Arc (LNBits) confirmed processor swap capability and webhook on settlement — payload shape TBC.*
...
## Session queue (post CC-70)

**Block 3** (franchise dashboard completion) → **Block 5** (merchant onboarding, first 1–2 merchants live) → **Block 8** (fiat → sats rewards) → **Session A** (CDK mint architecture) → **Session B** (stamp lifecycle + FCA) → **Block 9** (LNBits — post merchant onboarding) → **Ticketing MVP**
