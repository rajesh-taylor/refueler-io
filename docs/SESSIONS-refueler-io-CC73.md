...
- `{{ wordmarkSection or "Legend" }}` default was hardcoding "Legend" on every page that omitted `wordmarkSection` in frontmatter — homepage, support, editorial, notes, privacy all showed `REFUELER / LEGEND`
- Fixed: breadcrumb section (`/ SECTION`) now only renders when `wordmarkSection` is explicitly set in page frontmatter
- Legend page unaffected — already passes `wordmarkSection: "Legend"`
- Commit `9f7f33d`

**Item 2 — Main site `src/support/index.njk` — email, copy, theme script:**
- `privacy@refueler.io` → `support@refueler.io` in all user-facing positions (body copy, contact panel, footer email link)
- `privacy@` retained as secondary option in contact panel for GDPR queries — correct
- Inset blockquote rewritten: removed app-specific "missed order / reward" references — now generic across all Refueler products
- "What can I raise?" list items 1 and 2 genericised — previously named app-only scenarios
- Inline theme script updated: `localStorage` + `classList.add('carbon-mode')` → `rs-theme` cookie scoped to `.refueler.io` + `dataset.theme` — consistent with CC-72 pattern
- Footer email link updated to `support@refueler.io`
- Commit `9f7f33d`

**Item 3 — Share `src/_includes/nav.njk` — new link set:**
- Removed: App, Editorial, Privacy (wrong domain, wrong audience)
- Added: Notes (`refueler.io/notes/`), Support (`refueler.io/support/`)
- Kept: Upgrade (`/upgrade.html`), theme pill
- Wordmark stays `REFUELER / SHARE`
- Legend deliberately omitted — not fully live
- Commit `8a67ef0` on `refueler-share`

**Item 4 — Share `src/_includes/head.njk` — theme script migration:**
- `localStorage` + `rfTheme` key removed entirely
- `rs-theme` cookie, `.refueler.io` scoped, 30-day rolling, `SameSite=Lax`
- `dataset.theme` attribute only — no `classList` pattern remaining
- Cross-domain theme persistence confirmed working between `refueler.io` and `share.refueler.io`
- Commit `8a67ef0` on `refueler-share`

**Item 5 — REFUELER-BRIDGE.md v1.4:**
- AP-8 session log added — documents all changes above and states explicitly what Share-Master-Context.md and share-sessions.md must record
- Header updated: `refueler-multi-core` removed, `refueler-legend` added as fourth repo home
...
- GitHub repo renamed last night (separate Share session)
- Local path: `/Users/rajeshtaylor/Documents/refueler-legend/`
- All bridge copies verified clean — zero remaining references to `refueler-multi-core`
- MasterContext repo table updated

**Carry-forward:**
- Share ad hoc context sync — standalone session in Share project, attach REFUELER-BRIDGE.md v1.4, update Share-Master-Context.md and share-sessions.md from AP-8 section, commit
- Inline `<style>` block in `src/index.njk` — old nav/footer CSS with `html.carbon-mode` selector still present (dead code, safe to defer)
- New Anthropic API key for `refueler_csuite_briefing_v2_4.html` — generate fresh, store outside repo
- `blink-webhook_index.ts` — stale Svix version, delete or replace with v12 source
- GitHub Actions red X on `9b9655d` — non-fatal, fix when convenient
...
- `Costa Coffee HQ` category label: `Franchise_hq` → proper display name
- One Blink-to-Blink E2E test outstanding
...
**Next session:** Share ad hoc context sync (as above), then Block 3 — franchise dashboard completion.
