...
*Updated: 2026-08-08 (CC-80 close)**
*Supersedes: Refueler_MasterContext_IO_CC79b.md**
*Sync log: MasterContext_IO_CC80 — CC-80: nav destination pages restored (editorial, legend, support, privacy); :root blocks stripped from looks-done-isnt-done and the-float; inline theme script removed from looks-done-isnt-done; index.njk upload collision note added to file naming rule.*
...
**File naming rule (locked CC-74):** All `index.njk` files produced by Claude must be named with a section prefix — e.g. `home-index.njk`. Applies to `index.njk` files only. When uploading multiple prefixed files for review, upload one at a time — Claude only sees the last file if all share the same name. Rename to `index.njk` via `mv` before committing; the directory provides uniqueness, not the filename.
...
| Issue | Status |
|---|---|
| Nav links broken on refueler.io — Legend, Editorial, Privacy, footer links | ✅ Fixed CC-80 |
| Share nav — Support link broken | ✅ Fixed CC-80 |
| `looks-done-isnt-done` + `the-float` `:root` blocks | ✅ Fixed CC-80 |
...
|---|---|---|---|
| ~~CC-80 Part 1~~ | Nav fix | ✅ Closed `ee3584b` | |
| ~~CC-80 Part 2~~ | Editorial :root strip | ✅ Closed `2566bbf` | |
