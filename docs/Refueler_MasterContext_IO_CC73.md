# Refueler Master Context — IO CC-73
*Updated: 2026-08-04 (CC-73)*
*Supersedes: Refueler_MasterContext_IO_CC72.md*
*Sync log: MasterContext_IO_CC73 — Nav breadcrumb fix (wordmarkSection conditional); support page email corrected (privacy@ → support@), copy genericised; Share nav rewritten (Notes/Upgrade/Support); Share head.njk theme script migrated to rs-theme cookie; REFUELER-BRIDGE.md v1.4 committed to refueler-share, refueler-io (docs/), refueler-legend; refueler-multi-core renamed to refueler-legend on GitHub — confirmed across all bridge copies.*
...
**Share ad hoc context sync** (standalone session in Share project — attach REFUELER-BRIDGE.md v1.4, instruct to update Share-Master-Context.md and share-sessions.md from AP-8 section, commit) → **Block 3** (franchise dashboard completion) → **Block 5** (merchant onboarding, first 1–2 merchants live) → **Block 8** (fiat → sats rewards) → **Session A** (CDK mint architecture) → **Session B** (stamp lifecycle + FCA) → **Block 9** (LNBits — post merchant onboarding) → **Ticketing MVP**
...
## Nav architecture — locked CC-73

**Main site (`refueler.io`):** Ecosystem nav. Legend, Editorial, Notes, Privacy, theme pill. No Upgrade link — Upgrade is Share-specific.

**Share (`share.refueler.io`):** Product nav. Notes, Upgrade, Support, theme pill. Privacy footer-only. Status footer-only.

**Wordmark breadcrumb rule (locked CC-73):** `wordmarkSection` frontmatter variable controls the `/ SECTION` breadcrumb in `nav.njk`. Only set it on pages where the section label is meaningful (e.g. Legend page sets `wordmarkSection: "Legend"`). All other pages omit it — wordmark renders as plain `REFUELER`.

**Support email (locked CC-73):** `support@refueler.io` is the correct contact address for all user-facing support. `privacy@refueler.io` is retained for GDPR/data protection queries only. Both are on Tuta Business Essential. The support page (`/support/`) now correctly reflects this distinction.

**`refueler-multi-core` renamed to `refueler-legend` (CC-73):** GitHub repo renamed. Local path is now `/Users/rajeshtaylor/Documents/refueler-legend/`. All three copies of `REFUELER-BRIDGE.md` updated and confirmed clean — no remaining references to `refueler-multi-core`.
